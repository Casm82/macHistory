// Получает список MAC адресов через snmp c коммутатора для определённого vlan
// dot1dTpFdbStatus: other(1), invalid(2), learned(3), self(4), mgmt(5)
// session.subtree(MAC table) ---> subtree(ifStatus table) ---> Emit
module.exports = function (exchangeEmitter, switchObj, vlanObj) {
  ...
  var session = snmp.createSession (switchObj.ipaddress, config.community + "@" + vlanObj._id, options);

  ////////////////////////////////////////////////////////////////////////////////
  var oid = "1.3.6.1.2.1.17.4.3.1.2";    // таблица MAC-адресов
  // snmpbulkwalk -v 2c -c community@80 -OXqs 192.168.1.240 .1.3.6.1.2.1.17.4.3.1.2
  ...
  // snmpbulkwalk -v 2c -c community@80 -OXqs -m BRIDGE-MIB 192.168.1.240 .1.3.6.1.2.1.17.4.3.1.3
  ...
  function ifStatDoneCb (error) {
    /*if (error) {
      console.error("Ошибка SNMP: vlan: %s, switch: %s, error: %s", vlanObj._id, switchObj.ipaddress, error.message);
    }*/
    //console.log(switchObj);
    liveMACsArr.forEach(function(macInfo){
      //console.log("\nmacInfo ifIndex: %d", macInfo.ifIndex);
      //console.log(switchObj.collectMethod.excludePorts);
      liveIfStatArr.forEach(function(ifStat){
        if (
            (ifStat.switchId == macInfo.switchId) &&
            (ifStat.macDec == macInfo.macDec) &&
            (ifStat.vlanId == macInfo.vlanId) &&
    ...
    //console.log("\nvlan: %s, switch: %s - %s", vlanObj._id, switchObj.ipaddress, switchObj._id);
